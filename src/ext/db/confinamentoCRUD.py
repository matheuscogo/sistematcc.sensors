...
def cadastrarConfinamento(args):  # Create
    try:
        ...
        dataConfinamento = datetime.strftime(
            datetime.fromtimestamp(dataConfinamento/1000.0), '%d/%m/%y')
...
def canOpenDoor(matrizId):
    try:
        hasInseminacao = db.session.query(Inseminacao.Inseminacao.query.filter_by(
            confinamentoId=matrizId, active=True).exists()).scalar()
        ...
        confinamento = db.session.query(Confinamento.Confinamento).filter_by(
            matrizId=matrizId, active=True).first()
        canOpen = db.session.query(Aviso.Aviso).filter_by(
            confinamentoId=confinamento.id, active=True).first()
